...
- Append only the user-requested number of bounded derived directions to `directions.json`. Give each a unique, first-class `id`; never reuse an arbitrary primary direction ID. Record non-empty `derived_from_ids` and `combined_properties` mapping each combined or varied property to its source. Retain the complete direction contract, including linked official evidence, `baseline_exceptions`, all six axes, risks, and trade-offs. A new direction must still pass the three-axis pairwise diversity rule.
