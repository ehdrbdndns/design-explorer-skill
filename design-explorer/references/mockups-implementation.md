...
3. Create exactly one current pending entry per approved direction in `mockup-manifest.json` before calling the image tool. Record `direction_id`, `status`, `viewport`, `prompt_digest`, and a positive integer `attempt_count`. The list must remain within `run.json`'s `generation_budget` (default 5); attempts must remain within `max_attempts_per_direction` (default 2).
...
6. On the next user turn, replace pending entries with the returned safe relative local output path or conservative provider artifact hint in `output_ref`, plus `status`. Never store URL userinfo or credentials. Keep failure details. Permit one technical retry per failed direction; ask before additional variations, more attempts, or a larger image budget. Record an approved expansion on the `directions_approved` transition with `--generation-budget`, `--max-attempts-per-direction`, and `--approve-budget-expansion`.
...
- **Revise:** Only from `mockups_generated`, run `python3 scripts/run_state.py revise --run <run-dir> --reason "<user-request>"`. This archives the old manifest as `mockup-manifest.revision-<n>.json`, records the audit reason/time/count, clears selection and approvals, and returns to `directions_pending_approval`.
