...
8. Run the research validator and then the directions validator. Transitions rerun validation, so artifacts changed while awaiting approval cannot bypass the gate. Research evidence informs the design but never substitutes for implementation topology, recursive dependency, or rendered viewport evidence. Present `reference-board.md`, the evidence summary, and the mood directions together for review. In the response, present every direction as a complete block with:
