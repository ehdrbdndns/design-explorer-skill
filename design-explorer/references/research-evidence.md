...
3. Open the direct source behind each useful result. Record its direct public HTTP(S) URL, relevance, capture time, and concrete observations for layout, typography, palette, density, imagery, and interaction in `references.json`. Reject URL credentials, localhost/`.local`, and literal non-public IP hosts. Use `capture_path` only for a safe, sanitized relative artifact path without traversal or credentials.
