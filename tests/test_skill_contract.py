...
class SkillContractTests(unittest.TestCase):
    ...
    def test_single_generation_ledger_and_durable_lock_are_documented(self):
        ...
        for phrase in (
            ...
            "directory fd",
            "posix flock",
            "canonical runs-root directory fd",
            "global runs-root mutex",
            "rename-clone aba",
            "account home directory inode",
            "pwd.getpwuid(os.getuid()).pw_dir",
            "not `$home`",
            "root-owned and not group- or world-writable",
            "global serialization",
            "runs-root and run-directory locks remain defense in depth",
            "if mockup-manifest.json exists",
            "every load and status",
            "absence is allowed before first authorization",
            "path-to-fd identity",
            "legacy .generation.lock is never read, truncated, unlinked, or replaced",
            "diagnostic lock metadata is non-authoritative",
            "kernel releases the flock",
            "staged json is parsed and matched before replace",
            "post-replace",
            "byte-identical prior ledger",
            "linearization point",
            "cooperating design explorer processes",
            "cannot guarantee pathname contents remain unchanged",
            "arbitrary same-uid mutation at any time",
            "next load or validation",
            "short writes",
